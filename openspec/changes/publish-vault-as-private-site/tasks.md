...
- [x] 4.3 Build the note index that maps Obsidian note names to published pages, mirroring Obsidian's name-based resolution; verify tests cover a unique name, a name colliding across folders, and a name that does not exist
...
- [x] 5.7 Make pages readable at mobile widths, with wide tables handled without breaking layout; verify by inspecting the rendered output at a narrow viewport
...
- [x] 6.4 Verify end to end against a fixture vault that the output directory contains exactly the expected pages and nothing else

## 7. Distribution and deployment

- [x] 7.1 Write `action.yml` as a composite action running `npm ci` and the CLI from `$GITHUB_ACTION_PATH`, with no committed `dist/`; verify a workflow in this repo consumes the action against a fixture vault and produces output
- [x] 7.2 Tag the first release as `v1` and verify `uses: emmz-makes-stuff/vault-publisher@v1` resolves from another repository
- [x] 7.3 Add the `wrangler` configuration — `name`, `compatibility_date`, and no Worker script (`main` is optional for an assets-only Worker) — and verify `wrangler deploy` uploads a fixture site to the Worker. The assets directory is supplied **at deploy time** as `--assets`, from the action's `output-dir` output: the build output lands outside the workspace by design (section 6 refuses an output directory inside the vault root, which is what makes 7.7 structurally true), wrangler resolves `assets.directory` relative to the configuration file, and it does not interpolate environment variables — so a committed `assets.directory` cannot name that path. The configuration MUST carry `workers_dev: false` **and** must not enable `preview_urls` — preview URLs follow the `workers_dev` setting only unless explicitly configured, so a config enabling them re-opens the second bypass hostname while satisfying the first flag. Set `preview_urls: false` **explicitly** rather than relying on it inheriting `workers_dev`. The configuration is the authority, not a record of dashboard state: Cloudflare's documented behaviour is that a `workers.dev` route disabled **in the dashboard** is _re-enabled on the next `wrangler deploy`_ if the config omits `workers_dev: false`, and the same holds for preview URLs — so a first deploy from a config missing these flags actively undoes what 2.2 established, as a side effect of deploying. Verify both _before_ the first deploy, and re-request the `workers.dev` address **and** a preview address afterwards to confirm the deploy did not re-enable the routes 2.2 disabled. This applies to **every** wrangler configuration that deploys this Worker — if the fixture deploy here and the vault workflow's deploy use different config files, both carry the flags
- [x] 7.4 Add `.github/workflows/publish.yml` to the vault repository, triggered `on: push: branches: [main]`, running the action and deploying with `wrangler`; verify a push to main publishes and a push to another branch does not. The workflow's own deploy is a second, unattended path that runs on every push, so it re-checks the bypass hostnames itself rather than relying on 7.3's one-time check — see 7.8
- [x] 7.5 Store the Cloudflare deploy token as a secret in the vault repository and verify the workflow authenticates without it appearing in logs
...
- [x] 8.5 Tag a new `v1` on the publisher so the vault's workflow picks the change up, and verify the vault's next run renders a front page — the action is consumed as `@v1` and nothing reaches the vault until the tag moves
